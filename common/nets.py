...
class EnsembleRSSM(common.Module):
    ...
    def kl_loss(self, post, prior, forward, balance, free, free_avg, weights=None):
        kld = tfd.kl_divergence
        sg = lambda x: tf.nest.map_structure(tf.stop_gradient, x)
        lhs, rhs = (prior, post) if forward else (post, prior)
        mix = balance if forward else (1 - balance)
        if balance == 0.5:
            kl = kl_value = kld(self.get_dist(lhs), self.get_dist(rhs))
            if weights is not None:
                kl = kl * weights
            loss = tf.maximum(kl, free).mean()
        else:
            kl_lhs = kl_value = kld(self.get_dist(lhs), self.get_dist(sg(rhs)))
            kl_rhs = kld(self.get_dist(sg(lhs)), self.get_dist(rhs))
            if weights is not None:
                kl_lhs = kl_lhs * weights
                kl_rhs = kl_rhs * weights
            if free_avg:
                loss_lhs = tf.maximum(kl_lhs.mean(), free)
                loss_rhs = tf.maximum(kl_rhs.mean(), free)
            else:
                loss_lhs = tf.maximum(kl_lhs, free).mean()
                loss_rhs = tf.maximum(kl_rhs, free).mean()
...
class MLP_meta(common.Module):
    ...
    def __call__(self, features):
        ...
        x = x.reshape([-1, x.shape[-1]])
        for index in range(self._layers):
            x = self.get(f'dense{index}', tfkl.Dense, self._units, dtype=tf.float32)(x)
            x = self.get(f'norm{index}', NormLayer, self._norm)(x)
            x = self._act(x)
        x = self.get(f'dense_out', tfkl.Dense, np.prod(self._shape), dtype=tf.float32)(x)
        x = self.get(f'norm_out', NormLayer, self._norm)(x)
        x = tf.nn.sigmoid(x) / 2 + 0.75
        x = x.reshape(features.shape[:-1] + [x.shape[-1]])
        return x
...
class NormLayer(common.Module):
    ...
    def __call__(self, features):
        if not self._layer:
            return features
        return self._layer(features, training=True)
